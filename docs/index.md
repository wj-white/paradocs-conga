...
To enable/disable \\(\KaTeX\\) support inside the Paradocs theme, update `config.toml`:
...
With Mermaid support enabled in Paradocs, you can include the text definition of a Mermaid diagram inside a code block, and it will automatically be rendered by the browser as soon as the page loads.
...
SVG and PNG files exported with the tool contain the source code of the original diagram by default, which allows  the diagrams.net site to import those images again for edit in the future.  Paradocs can detect this and automatically add an "edit" button over any image that can be edited using the online site.
...
As the diagram data is transported via the browser, the diagrams.net server does not need to access the content on your Paradocs server directly at all.


{{< figure src="paradocs-conga-diagrams.svg" caption="Mouse over the above image and click the edit button!">}}
